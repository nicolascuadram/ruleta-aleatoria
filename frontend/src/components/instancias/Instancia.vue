...
const roulette_content = ref([]);

const updateRouletteContent = (content) => {
    roulette_content.value = content;
	console.log(`Contenido para la ruleta: ${roulette_content.value}`);
};
...
<template>
	<div class="flex justify-between items-start w-full h-full">
		...
		<div class="basis-2/4 grow h-full">
			<!-- Ruleta Aleatoria -->
			<!-- <Ruleta :id="id" /> -->
			<Ejecucion @update:roulette_content="updateRouletteContent" />
		</div>
		<!-- Lado Derecho -->
		<div class="basis-1/4 grow h-full">
